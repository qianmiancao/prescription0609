...
class KnowledgeManager:
    ...
    def upload_docs(self, file_path, original_name):
        ...
        docs = loader.load()
        # 【修复点 1】强制纠正 metadata 中的 source 为原始文件名，不记录临时路径
        for doc in docs:
...
class PharmacyAgent:
    ...
    def audit(self, prescription_json, context):
        # 【修复点 3】在 Prompt 中明确禁止使用 Markdown 符号
        system_prompt = """你是一位资深临床药师。请为【处方数据】输出正式报告。
        
        重要格式要求：
        1. 禁止使用任何 Markdown 符号，特别是禁止使用星号（*）进行加粗或列表。
        2. 使用纯文本序号（如 1. 2. 3. 或 1.1, 1.2）来划分段落。
        3. 请直接输出文字，保持排版整洁。
        4. 报告末尾列出参考的文件名。"""
...
def main():
    ...
    with col_in:
        ...
        if st.button("🔍 执行深度点评", type="primary", use_container_width=True):
            if api_key:
                ...
                with st.spinner("正在检索并清理格式中..."):
                    context_str, sources = km.retrieve_context_with_sources(med_df["药品名称"].tolist())
                    st.session_state.current_sources = [os.path.basename(s) for s in sources]
    ...
    with col_out:
        ...
        if st.session_state.current_sources:
            with st.expander("📂 本次点评参考的本地文件 (已过滤路径)", expanded=True):
        ...
        if st.session_state.report_content:
            ...
            with c1:
                ...
                st.download_button("📥 导出 Word (纯净版)", data=docx_data, file_name="处方点评报告.docx", use_container_width=True)
            with c2:
